...
from models.models import lstm_v1, lstm_v2, lstm_v3, lstm_v4, lstm_v5, lstm_v6, cnn_v1, cnn_v2, cnn_v3, cnn_v0, \
    best_cnn, bnn_v1, hybrid_v1, bert_v1
...
def train_net(preprocessor, reshape=False, split = False):

    if preprocessor is DisasterProcessor:
        preprocessor = DisasterProcessor()
        feature = "text"
    elif preprocessor is EmotionProcessor:
        preprocessor = EmotionProcessor()
        feature = "Sentence"
    ...
    if split and isinstance(preprocessor, DisasterProcessor):
    ...
    if isinstance(preprocessor, DisasterProcessor):
        ...
        X_test = test_data[0]
        sequences = tok.texts_to_sequences(X_test[feature].tolist())
        X_test = pad_sequences(sequences, maxlen=max(len_train_words))
    ...
    if isinstance(preprocessor, DisasterProcessor):
        ...
        pass
        # y_preds = [emotions[x] for x in y_preds]
        #
        # res = pd.DataFrame(test_data[feature], columns=[feature])
        #
        # res["result"] = y_preds
        #
        # res.to_csv("data/FinalEmotion.csv")
